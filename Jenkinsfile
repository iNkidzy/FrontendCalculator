pipeline{
     ...
     stages {
         ...
         stage("Execute system tests") {
             steps {
                 sh "slenium-side-runner-server http://localhost:4444/wd/hub -c 'browserName=firefox' --base-url http://app-test-container test/system/FunctionalTests.side"
                 sh "slenium-side-runner-server http://localhost:4444/wd/hub -c 'browserName=chrome' --base-url http://app-test-container test/system/FunctionalTests.side"
             }
         }
     }
    post {
